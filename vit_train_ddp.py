...
def train_one_epoch(model, criterion, optimizer, scheduler, dataloader, device, epoch, writer, args, logger):
    model.train()
    ...
    for batch_idx, (data, label) in enumerate(dataloader):
        ...
        if batch_idx % 20 == 0 and logger:
            logger.info(f"Epoch {epoch} Batch {batch_idx}: Loss = {loss.item():.4f}")

def evaluate(model, criterion, dataloader, device, writer, step, args, logger):
    model.eval()
    total_loss = 0
    with torch.no_grad():
...
def main(args):
    ...
    logger = setup_logger(args.output_path, args.rank)
    if logger:
        logger.info("Starting DDP training with InversionViT")
    ...
    dataloader_train = DataLoader(dataset_train, batch_size=args.batch_size, sampler=train_sampler, num_workers=args.workers)
    dataloader_val = DataLoader(dataset_val, batch_size=args.batch_size, sampler=val_sampler, num_workers=args.workers)
    ...
    writer = SummaryWriter(log_dir=os.path.join(args.output_path, 'logs')) if args.tensorboard and args.rank == 0 else None

    best_loss = float('inf')
    for epoch in range(args.epochs):
        train_sampler.set_epoch(epoch)
        train_one_epoch(model, criterion, optimizer, scheduler, dataloader_train, device, epoch, writer, args, logger)
        val_loss = evaluate(model, criterion, dataloader_val, device, writer, step=epoch, args=args, logger=logger)

        if val_loss < best_loss and args.rank == 0:
            best_loss = val_loss
            torch.save(model.module.state_dict(), os.path.join(args.output_path, 'best_model.pth'))
            logger and logger.info(f"Model saved at epoch {epoch} with val loss {val_loss:.4f}")
